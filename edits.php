...
if(isset($_REQUEST["btn"])){
    ...
    $result=mysqli_query($dbh,"UPDATE student SET Name='$nname',Fname='$nfname',Qualification='$nqual',Sex='$nsex',DOB='$ndob',Contact='$ncontact',Password='$npassword' WHERE Adm_no=$id");  
    ...
    $id=$row['Sno'];
    $role=$row['Role'];
    $row2=mysqli_query($dbh,"SELECT * FROM student WHERE Adm_no=$id AND $role='Student'");
    if (!$result){     
        
        echo '<script> alert("Failed to Update!"); window.location.href="edits.php"</script>';
    }
    else{
        if($row2->num_rows == 0){
            echo '<script> alert("Successfully Updated!"); window.location.href="modifys.php"</script>';
        }
        else{
            echo '<script> alert("Successfully Updated!"); window.location.href="mps.php"</script>';
        }
    }
}
mysqli_close($dbh);
